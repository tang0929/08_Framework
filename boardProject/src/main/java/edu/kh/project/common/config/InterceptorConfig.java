...
public class InterceptorConfig implements WebMvcConfigurer{
	...
	// 인터셉터 클래스 Bean 등록
	@Bean // 개발자가 만들어서 반환하는 개체를 Bean으로 등록함. 관리는 Spring Container가 수행
	...
	public void addInterceptors(InterceptorRegistry registry) {
		...
		.addPathPatterns("/**") // 가로챌 요청 주소를 지정. 여러 주소 작성시 "","" 컴마로 구분
		// /** : /이하 모든 요청 주소
	
		
		.excludePathPatterns("/css/**","/js/**","/images/**","/favicon.ico"); // 가로채지 않을 주소 지정
		
		// Bean으로 등록된 BoardNameInterceptor 객체를 얻어와 인터셉터에 등록

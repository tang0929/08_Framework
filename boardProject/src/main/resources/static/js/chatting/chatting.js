...
const sendMessage = () => {
  ...
  const chattingObj = {
    ...
    "targetNo" : selectTargetNo, // 메시지를 받을 대상의 회원 번호
    "messageContent" : msg,   // 전달할 메시지 내용
    "chattingNo" : selectChattingNo  // 채팅방 번호(DB 저장 용도)
  }
    ...
    document.querySelector("#inputChatting").addEventListener("keyup", e => {
        ...
        if(e.key == "Enter") {
            ...
            if(!e.shiftKey) {
                ...
                // shift가 눌러지지 않은 경우 == shift + enter 입력 시 제출이 아닌 줄바꿈
...
// 사용자 검색(ajax)
targetInput.addEventListener("input", e => {   // input : 붙여넣기, 키보드 입력 등 입력 방법에 제한을 두지 않는 입력

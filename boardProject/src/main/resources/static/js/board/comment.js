...
- 자원(데이터, 파일 등)에 이름(주소)으로 구분(representational)하여
    자원의 상태(State)를 주고 받는 것(Transcfer)

- 자원의 이름(주소)를 명시하고 HTTP Method(GET, POST, PUT, DELETE)를 이용하여 지정된 자원에 대한 CRUD 진행 
...
const selectCommentList = () => {
    ...
    /* fetch는 JS에서 제공해주는 비동기 기본 기능
    fetch(주소, {method = "", header = {}, body : ""} : [POST, PUT, DELETE]
    fetch(주소?쿼리스트링) : [GET] 
    ...
    .then(commentList => {
        ...
        for(let comment of commentList){
            ...
            if(comment.commentDelFl == 'Y'){
                    ...
                    // 부모 댓글이 삭제되지 않은 댓글일 경우 작성자, 프로필 이미지, 날짜 감싸는 요소가 표시
                    ...
                    // 프로필 이미지가 없으면 기본 이미지로 설정 있으면 회원 이미지
